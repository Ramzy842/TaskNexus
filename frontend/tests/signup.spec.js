...
const signup = async (page, username, name, email, password) => {
...
test("Signup with valid username length: 3, valid email, name and password via signup form.", async ({
    ...
    await signup(page, "Val", "Drax", "validemail@gmail.com", "password123456");
    const successMsg = await page.getByText(
        "Registration completed successfully."
    );
    await expect(successMsg).toBeVisible();
...
test("Signup with random invalid credential via signup form", async ({
    ...
    await signup(page, "RobertDoe", "Robert Doe", "robert-doe@gmail.com", "");
...
test("Signup with a duplicate email that has different case sensitivity than the original email via signup form.", async ({ page }) => {
    await signup(
        page,
        "Loki_GOS",
        "Loki Laufeyson",
