...
const PasswordChange = ({ loading }) => {
    ...
    const handleErrors = (errors) => {
        ...
        let newPassErrors = errors.filter(
            (error) =>
                /\bnew\b/i.test(error) || /\bPassword Should\b/i.test(error)
        );
...
const Settings = () => {
    ...
    useEffect(() => {
        document.title = "TaskNexus | Settings";
    }, []);
    return (
        <DashboardLayout>
            <h1 className="font-medium text-xl text-teal-900 mb-4">
                My Account
            </h1>
            {loading ? (
                <SkeletonSettings />
