  ...
  const handleStatusChange = (newStatus) => {
    setTaskDetails((prevDetails) =>
      prevDetails.map((detail) =>
        detail.type === "select" ? { ...detail, value: newStatus } : detail
      )
    );
  };
  const handleSave = () => {
          ...
          <>
            <div className="sm:w-1/2 sm:max-w-1/2 mb-16 sm:mb-0">
              {taskDetails.map((detail, index) => {
                const { id, placeholder, iconSrc, type, value, target } =
                  detail;
                return (
                  <TaskInput
                    editMode={true}
            ...
            <div>
              <div
                onClick={handleSave}
                className="flex justify-center items-center bg-cyan-600 hover:bg-cyan-700 rounded-sm p-3 cursor-pointer mb-2 w-full"
              >
                <svg
                  width="24"
                  height="24"
